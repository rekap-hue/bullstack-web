            ...
            <div className="flex-grow p-4 md:p-8 flex flex-col justify-center relative">
              ...
              <div className="flex items-center gap-3 md:gap-6 mb-2 relative z-10 pl-2 md:pl-3">
                ...
                <div className="relative select-none" style={{ transform: g.active ? `translate(${g.shakeX}px,${g.shakeY}px)` : 'none', transition: g.active ? 'none' : 'transform 0.15s ease' }}>
                  ...
                  {g.active && <h2 className="absolute inset-0 text-3xl md:text-5xl font-black italic tracking-tighter uppercase text-cyan-400 mix-blend-screen pointer-events-none" style={{ transform: `translate(${g.cShift[0]}px,${g.cShift[1]}px)`, opacity: 0.3 }} aria-hidden>{displayText}</h2>}
                  {g.active && <h2 className="absolute inset-0 text-3xl md:text-5xl font-black italic tracking-tighter uppercase text-red-500 mix-blend-screen pointer-events-none" style={{ transform: `translate(${g.rShift[0]}px,${g.rShift[1]}px)`, opacity: 0.25 }} aria-hidden>{displayText}</h2>}
                  {g.active && g.slices.map((sl, i) => (
                    <h2 key={i} className="absolute inset-0 text-3xl md:text-5xl font-black italic tracking-tighter uppercase text-orange-300 pointer-events-none" style={{ clipPath: `inset(${sl.top}% 0 ${Math.max(0, 100 - sl.top - sl.height)}% 0)`, transform: `translateX(${sl.shiftX}px)`, opacity: 0.7 }} aria-hidden>{displayText}</h2>
                  ))}
                </div>
              </div>
...
const BullStackQuestionLogo = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      ctx.rotate(sway);
      ctx.font = 'bold 130px "Courier New", Courier, monospace';
    ...
    >
      ...
      <section className="relative z-20 mx-auto w-full max-w-6xl px-4 md:px-6">
        ...
        >
          ...
          <span
            className="relative block overflow-hidden border border-orange-400/70 bg-black px-7 py-4.5 text-orange-200 shadow-[0_0_30px_rgba(249,115,22,0.28)] transition-all duration-300 group-hover:scale-[1.005] group-hover:shadow-[0_0_42px_rgba(249,115,22,0.4)] md:px-12 md:py-5.5"
            style={{
              clipPath:
          ...
          >
            ...
            <span className="relative flex items-center justify-center gap-4 md:gap-6">
              <span className="text-center text-[24px] font-black italic tracking-tighter uppercase leading-none whitespace-nowrap text-orange-500 md:text-[52px]">
                VYPUSTIT PROJEKT
              </span>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              ...
              <div className="flex items-center justify-center h-36 mb-3">
                <img src="/iron-core-logo.png" alt="Iron Core" className="w-56 h-56 object-contain opacity-90 group-hover:opacity-100 transition-opacity duration-300 drop-shadow-[0_0_12px_rgba(236,72,153,0.4)]" />
              </div>
